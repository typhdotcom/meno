...
| **G6 — binding** ([Binding](#binding)) | At `b₁ = 2`, the two basis sectors attract exactly when their cycles overlap with consistent orientation | law `binding_attractive_iff` on the intrinsic `bindingEnergyClass`; closed form `ofCycles_bindingEnergy_fin_two`; strictness `theta_binding_attractive_class`; boundary `wedge_binding_zero` |
...
| `binding_attractive_iff`, `theta_binding_attractive_class` | **The binding sign criterion** (G6): on a rank-two cycle lattice (`b₁ = 2`), the two basis sectors attract **exactly when their cycles overlap with consistent orientation** — `0 < bindingEnergyClass ↔ 0 < ⟨c₁,c₂⟩`. The left side is intrinsic (`bindingEnergyClass`, computed by every chart), so the sign is forced by topology, not by choice of basis. Strictness witness: the intrinsic binding of the theta classes is positive, their cycles overlapping with `⟨c₁,c₂⟩ = 2` |
